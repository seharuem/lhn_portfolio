...
`,G=l.button.attrs({type:"button",className:"top fixed z-50 bottom-10 right-10 w-10 aspect-square max-lg:right-6 max-lg:bottom-6 rounded-full bg-(--main) text-(--bg) font-extrabold scale-0"})`
...
`,F=l.div.attrs({className:"page p-20 max-lg:p-10 flex flex-col gap-40 max-lg:gap-20 -mt-[100vh] opacity-0"})`
	@media (max-width: 895px) {
		opacity: 1 !important;
		margin-top: 0;
	}
`,J=b`
to {translate: -8px -2px}`,Y=l.button.attrs({type:"button",className:"skip absolute top-20 right-20 text-(--main) font-bold flex items-center"})`
	font-size: 24px;
	&::before {
		content: '';
		height: 32px;
